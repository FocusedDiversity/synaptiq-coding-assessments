...
const rawMonths: { [id: number]: IMonths } = {
  0: {
    name: 'January',
    shortName: 'Jan',
...
const rawDays: { [id: number]: ICalendarDay } = {
  0: {
    name: 'Sunday',
    shortName: 'Sun',
...
> = (state, action) => {
  switch (action.type) {
    case 'SET_INIT_STATE': {
      ...
      const weekday = rawDays[dayOfWeek].calendarWeekdayNumber;
      const displayDate = getRawDate(new Date(year, month, today.getDate()));
    ...
    case 'SET_DATE': {
      ...
      const displayDate = getRawDate(dateToFormat);
    ...
    case 'ADD_MONTH': {
      ...
      const firstWeekdayNumber =
        rawDays[newMonthFirstWeekdayNumber].calendarWeekdayNumber;
...
const getRawDate = (date: Date): string => {
  const year = date.getFullYear();
  const monthShortName = rawMonths[date.getMonth()].shortName;
  const day = ('0' + date.getDate()).slice(-2);
  const dayShortName = rawDays[date.getDay()].shortName;
...
export const DatePicker = () => {
  const [state, dispatch] = React.useReducer<
    React.Reducer<DatePickerReducerState, TDatePickeReducerAction>
